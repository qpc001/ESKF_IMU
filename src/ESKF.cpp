...
//    TicToc tic;
    ...
    // 更新协方差
    Eigen::MatrixXd Qc_dt= 0.5*dt*this->Qi;
...
//
    ...
    //重置误差状态为零，更新误差状态协方差P
    this->Fi.setIdentity(err_state.Vec.size(), err_state.Vec.size());
    Fi.block<3,3>(3, 3)= M3d::Identity() - skew_matrix(0.5 * errors.segment(3, 3));
    this->P = Fi * this->P * Fi.transpose();
}
